...
import { RefreshCw, ExternalLink, ThumbsUp, MessageCircle, Share2, Eye, Users } from 'lucide-react'
    ...
    >
      <div className="flex gap-4 p-4">
        ...
        {scheduledPost?.image_url ? (
          <div className="relative w-20 h-16 sm:w-28 sm:h-20 rounded-lg overflow-hidden flex-shrink-0">
            <Image src={scheduledPost.image_url} alt="post" fill className="object-cover" unoptimized />
          </div>
        ) : (
          <div className="w-20 h-16 sm:w-28 sm:h-20 rounded-lg bg-slate-100 dark:bg-slate-800 flex items-center justify-center flex-shrink-0">
        ...
        <div className="flex-1 min-w-0 space-y-1.5">
          ...
          <div className="flex items-center gap-3 pt-0.5">
            <span className="flex items-center gap-1 text-[11px] text-slate-500 dark:text-slate-400">
              <Eye className="w-3 h-3" />{post.initial_reach}
            </span>
            <span className="flex items-center gap-1 text-[11px] text-slate-500 dark:text-slate-400">
              <Users className="w-3 h-3" />{post.initial_impressions}
            </span>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-1 flex-shrink-0" onClick={e => e.stopPropagation()}>
          ...
          {post.post_id && (
            ...
            >
              <a href={`https://facebook.com/${post.post_id}`} target="_blank" rel="noopener noreferrer">
