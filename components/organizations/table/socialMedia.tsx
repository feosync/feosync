...
const ICONS: Record<string, React.ReactNode> = {
  ...
  threads: (
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 192 192" className="w-6 h-6 fill-white">
  ...
  bluesky: (
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 568 501" className="w-6 h-6 fill-white">
  ...
  mastodon: (
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 74 79" className="w-6 h-6 fill-white">
...
export function ManageSocialMedia({
  open,
  onOpenChange,
  orgId,
  ...
  // Canaux connectés récupérés via l'API
  const [connectedChannels, setConnectedChannels] = useState<SocialCardProps[]>([]);
  const [loadingChannels, setLoadingChannels]     = useState(true);

  // Charge les canaux connectés au montage (et si orgId change)
  useEffect(() => {
    let cancelled = false;
    ...
    const fetchChannels = async () => {
      ...
      try {
        ...
        // Fait correspondre les noms renvoyés par l'API avec le catalogue local
  ...
  const handleChannelSelect = (channel: SocialCardProps) => {
    ...
    onOpenChange(false); // ferme le picker
    setConnectOpen(true); // ouvre le dialog de connexion
  };
  ...
  // Icônes affichées dans la barre : les 3 derniers canaux connectés
  const previewChannels = connectedChannels.slice(-3);

  return (
    <>
      {/* ── Barre sociale ─────────────────────────────────────────────────── */}
      <div className="flex items-center justify-start p-2 rounded-2xl h-14 w-full relative">

        {/* Icônes des canaux connectés (max 3, empilées) */}
        {!loadingChannels &&
          previewChannels.map((ch, index) => {
            // Décalage à droite en partant du bouton « + » (right-4)
            // index 0 = le plus à gauche, index 2 = juste à gauche du bouton
            const rightOffset = 4 + (previewChannels.length - index) * 8 + (previewChannels.length - index - 1) * (-1);
            // right-12 → right-20 → right-28 pour 3 éléments
            return (
              <div
                key={ch.id}
        ...
        {loadingChannels &&
          [0, 1, 2].map((i) => (
            <div
              key={i}
              className="w-10 h-10 rounded-full bg-slate-200 dark:bg-slate-700 animate-pulse absolute z-0"
          ...
          <div className="grid grid-cols-4 p-2 max-h-[70vh] overflow-y-auto">
            {CHANNELS.map((ch) => (
              <button
                ...
                className="group flex flex-col items-center gap-3 p-4 rounded-xl text-center transition-colors duration-150 hover:bg-slate-100 dark:hover:bg-slate-800/60 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-slate-400 cursor-pointer"
              >
                <span
                  className={`flex items-center justify-center w-14 h-14 rounded-2xl shadow-sm ${ch.bg} transition-transform duration-150 group-hover:scale-105 relative`}
                >
                  {ch.icon}
                  {/* Badge « connecté » */}
                  {connectedChannels.some((c) => c.id === ch.id) && (
                    <span className="absolute -top-1 -right-1 w-4 h-4 rounded-full bg-green-500 border-2 border-white dark:border-slate-900" />
                  )}
                </span>
      ...
      {/* ── Dialog de connexion (conditionnel au canal sélectionné) ───────── */}
