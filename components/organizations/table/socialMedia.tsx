...
export function ManageSocialMedia({ open, onOpenChange, orgId }: ManageSocialMediaProps) {
  // ✅ Fix 1 : état React propre, initialisé à null — plus de variable module non initialisée
  ...
  const handleConnectOpenChange = (value: boolean) => {
    ...
    if (!value) setSelectedChannel(null); // reset à la fermeture
  };

  return (
    <>
      {/* Barre sociale */}
      <div className="flex items-center justify-start p-2 rounded-2xl  h-14 w-full">
        ...
        <DialogContent className="sm:max-w-lg p-0 gap-0 overflow-hidden rounded-2xl border border-slate-200 dark:border-slate-800">
          ...
          <div className="grid grid-cols-4 p-2 max-h-full overflow-y-auto">
            {CHANNELS.map((ch) => (
              ...
              >
                <span className={`flex items-center justify-center w-14 h-14 rounded-2xl shadow-sm ${ch.bg} transition-transform duration-150 group-hover:scale-105`}>
                  {ch.icon}
                </span>
                <span className="flex flex-col gap-0.5">
                  <span className="text-sm font-medium text-slate-800 dark:text-slate-100 leading-tight">{ch.name}</span>
                  <span className="text-[11px] text-slate-400 dark:text-slate-500 leading-tight">{ch.sub}</span>
                </span>
              </button>
            ))}
          </div>
      ...
      {/* ✅ Fix 2 : guard — rendu conditionnel, plus de crash sur undefined.isSocialOpen */}
