...
const HowItWorks = () => {
  ...
  return (
    <section className="py-24 px-6" ref={containerRef} id='commentçamarche'>
      <div className="max-w-3xl mx-auto">
        ...
        <div className="relative">
          {/* Timeline line */}
          <div className="absolute left-6 md:left-8 top-0 bottom-0 w-0.5 bg-google-border">
            <motion.div
              style={{ height: lineHeight }}
              className="w-full bg-gradient-to-b from-google-blue to-google-teal"
            />
          </div>
          ...
          <div className="space-y-12">
            {steps.map((step, i) => (
              ...
              >
                <div className="absolute left-3 md:left-5 w-6 h-6 rounded-full bg-google-blue flex items-center justify-center">
                  <span className="text-[10px] text-primary-foreground font-mono font-medium">
                    {step.num}
                  </span>
                </div>
                <h3 className="font-ui text-xl font-medium text-google-dark mb-2">{step.title}</h3>
                <p className="text-google-gray-ui leading-relaxed">{step.desc}</p>
              </motion.div>
            ))}
          </div>
        </div>
