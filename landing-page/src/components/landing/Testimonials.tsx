          ...
          {testimonials.map((t, i) => (
            ...
            >
              <div className="flex items-center gap-3 mb-4">
                <div className={`w-10 h-10 rounded-full ${t.color} flex items-center justify-center text-primary-foreground text-sm font-bold font-ui`}>
                  {t.initials}
                </div>
                <div>
                  <p className="font-ui font-medium text-sm text-google-dark">{t.name}</p>
                  <p className="text-xs text-google-gray-mid">{t.role}</p>
                </div>
              </div>
              <div className="flex gap-0.5 mb-3">
                {[...Array(5)].map((_, j) => (
                  <span key={j} className="text-google-teal text-sm">★</span>
                ))}
              </div>
              <p className="text-google-gray-ui text-sm leading-relaxed italic">"{t.quote}"</p>
            </motion.div>
          ))}
        </div>
      </div>
